...
const books = [
  ...
  // Menggunakan fungsi filter untuk mengembalikan nilai item books dengan sales > 1000000
  const filteredBooks = books.filter(book => book.sales > 1000000);
  
  // Menggunakan fungsi map untuk mengembalikan nilai string dengan format yang diinginkan
  const greatAuthors = filteredBooks.map(book => `${book.author} adalah penulis buku ${book.title} yang sangat hebat!`);
